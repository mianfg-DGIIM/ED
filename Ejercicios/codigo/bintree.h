...
// LEER ÁRBOL
void lee_arbol(std::istream& in, node * & nod){
  char c;
  in >> c;                     //Lectura de un carácter
  if (c=='n'){                 //Si es 'n'
    nod = new node;            //Reservamos un nuevo nodo
    in >> nod->etiqueta;       //Leemos la etiqueta
    lee_arbol(in,nod->elnodo->izqda);  //Leemos recursivamente el hijo izquierda
    lee_arbol(in,nod->elnodo->drcha);  //Leemos recursivamente el hijo derecha
    if (nod->izqda!=0)         //Si hay hijo izquierda,
      nod->elnodo.izqda->pad=nod;   //actualizamos su padre
    if (nod->drcha!=0)         //Si hay hijo derecha,
      nod->elnodo->dcha->elnodo->pad=nod;   //actualizamos su padre
  }
...
void escribe_arbol(std::ostream& out, node * nod) const{
...
friend ostream& operator<< (ostream& out, const bintree<T>& v){
  v.escribe_arbol(out, v.laraiz); //Escribe el árbol
  return out;                    //Devuelve out para permitir encadenamientos
}

friend istream& operator>> (istream& in, bintree<T>& v){
